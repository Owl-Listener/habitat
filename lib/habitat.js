 ...
 *   └── research/             optional
 *       ├── insights/         one finding per file, cited by id
 *       └── people/           optional: who the product serves
 *
...
export function loadDesign(dir) {
  ...
  const design = { root, principles: null, tokens: null, components: [], insights: [], people: [], errors: [] };
  ...
  // Every folder of one-file-per-thing works the same way.
  const readFolder = (folder, into) => {
    const dir = join(root, folder);
    if (!existsSync(dir)) return;
    for (const file of readdirSync(dir).sort()) {
      // Files starting with "_" (like _TEMPLATE.md) are scaffolding, not content.
      if (!file.endsWith(".md") || file.startsWith("_") || file.toLowerCase() === "readme.md") continue;
      const path = join(dir, file);
      try {
        into.push(readMarkdown(path));
      } catch (err) {
        design.errors.push({ path, message: `could not parse: ${err.message}` });
      }
    }
  };
  readFolder("components", design.components);
  readFolder("research/insights", design.insights);
  readFolder("research/people", design.people);

...
name:
  ...
  aliases:
    ...
    - field
purpose: >-
  Lets a user enter a short piece of free-form text. It is the primary control for collecting typed
  data such as names, email addresses, search terms, and passwords. For multi-line text, use a
  Textarea instead.
props:
  - name: label
    type: string
...
variants:
  ...
  - name: error
    whenToUse: >-
      After the user has submitted or left the field and the value fails validation. Always pair
      with an errorMessage that says what went wrong.
states:
  - name: default
    description: 'Resting state. Neutral border, white background.'
  - name: focus
    description: Keyboard or pointer focus. Border and outline shift to the focus-ring token.
  - name: disabled
...
tokens:
  ...
  focus.border: color.state.focusRing
  focus.ring: color.state.focusRing
  error.border: color.state.error
  error.text: color.state.error
...
relationships:
  ...
  pairsWith:
    ...
    - Select
antiPatterns:
  - never: Omit the label prop or replace it with only a placeholder.
    because: >-
...
accessibility:
  ...
  keyboard:
    ...
    - Enter may submit the enclosing form
  focus: >-
    Native focus order. Visible 2px focus ring on :focus-visible using the focus-ring token, offset
    by 2px. Focus ring colour shifts to error token when in error state.
  aria:
    - aria-invalid="true" when in error state
    - aria-describedby points to the error message element when errorMessage is set
    - >-
      label is always associated via htmlFor/id — never use aria-label as a substitute for a visible
      label
  contrast: >-
    Error red on white meets WCAG AA 4.5:1. Disabled text on disabled surface meets WCAG AA 4.5:1
    for the default token set.
content:
  examples:
    - Email address
    - Full name
...
Always pass a label that names the data being collected, not the action ('Email address', not 'Enter email'). When building a form, pair each Input with a Button for submission. Never generate a form where the only label context is inside the placeholder.
